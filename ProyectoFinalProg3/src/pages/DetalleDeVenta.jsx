...
const DetalleDeVenta = () => {
    ...
    const getDatos = async () => {
      ...
      const handleEliminarDetalle = async (idDetalle) =>{
        let response = await axios.delete(`http://localhost:8000/DetalleVentas/BorrarDetalleVenta/${idDetalle}`)
        if(response) {
    ...
    return (
        <div>
            ...
            <div className="btn-toolbar d-flex justify-content-center">
                <div className="btn-group me-2 item-align-center">                                            {/* con una clase de bootstrap creamos un grupo de botenes (Link) para navegar entre las diferentes tablas */}
                    <Link to='/productos' className='btn btn-primary'>Productos</Link>
                    <Link to='/clientes' className='btn btn-primary'>Clientes</Link>
                    <Link to='/ventas' className='btn btn-primary'>Ventas</Link>
                    <button type="button" className="btn btn-primary">Detalle de Ventas</button>
                    <Link to='/empleado' className='btn btn-primary'>Empleados</Link>
                    <Link to='/usuario' className='btn btn-primary'>Usuario</Link>
            ...
            <br />
            <div className="p-5">
                <Link to='/detalle/agregar' className='btn btn-success float-right'>Agregar</Link>
            </div>
            ...
            <div className='tabla'>
                <Table striped bordered hover>
                    ...
                    <tbody>
                        {datos.map ((datos)=> <tr key={datos.idDetalle}>
                            ...
                            <td>
                                <Link to={`/detalle/editar/${datos.idDetalle}`} className='btn btn-warning mr-3'><MdEdit/></Link>               {/* boton que nos envia a la pagina para editar un campo */}
                                <button className="btn btn-danger" onClick={()=>handleEliminarDetalle(datos.idDetalle)}><FaTrashAlt /></button></td>                        {/* boton para eliminar */}
                        </tr>)}
                    </tbody>
                </Table>
            </div>
